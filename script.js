...
function updateDeliveryOption() {
  ...
  if (selectedOption === "delivery") {
    ...
    showMessage(
      "Lieferoption ausgewählt. Bitte füllen Sie die Lieferdetails aus."
    );
  } else {
    deliveryCost = 0;
...
function checkout() {
  ...
  if (selectedOption === "delivery") {
    ...
    if (!name || !street || !postalCode || !city) {
      showMessage(
        "Bitte füllen Sie alle erforderlichen Felder aus: Name, Straße, Postleitzahl und Stadt."
      );
      return;
    }

    showMessage(
      "Danke für Ihre Bestellung! Ihre Lieferung wird in 30 Minuten eintreffen."
    );
  } else {
    showMessage(
      "Danke für Ihre Bestellung! Die Bestellung kann in 30 Minuten abgeholt werden."
    );
  }
